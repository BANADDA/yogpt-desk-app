...
const MainContent = () => {
  ...
  useEffect(() => {
    ...
    const fetchSystemUsage = async () => {
        try {
            const response = await fetch('http://localhost:3006/system-usage');
            if (response.ok) {
...
const handleStartProgram = async (data) => {
  try {
    ...
    const response = await fetch('http://localhost:3006/start-program', {
      method: 'POST',
      headers: {
...
const toggleProgramStatus = async () => {
  if (programStatus === 'online') {
    try {
      const response = await fetch('http://localhost:3005/stop-program', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
